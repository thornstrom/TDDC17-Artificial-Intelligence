...
public class QLearningController extends Controller {
	...
	final static int ACTION_ALL = 3;
	final static int ACTION_NONE = 4;
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				double qsa = Qtable.get(prev_stateaction);
				double nsa = alpha(Ntable.get(prev_stateaction)); /* learning rate, depends on the number of times action a is tested in state b*/ 
				double max_qsa =  getMaxActionQValue(new_state);
				
				/* The update equation for TD Q-learning */
				double updated_value = qsa + nsa *(previous_reward + GAMMA_DISCOUNT_FACTOR * max_qsa - qsa);
				Qtable.put(prev_stateaction, updated_value);
