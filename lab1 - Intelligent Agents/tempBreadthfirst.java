...
class Node {
	...
	public Node(int x, int y, int direction, ArrayDeque<Integer> list) {
		this.x = x;
		this.y = y;
	...
	public ArrayDeque<Integer> getList() {
		return list;
	}
...
class MyAgentProgram implements AgentProgram {
	...
	private void breadthFirstSearch() {
		...
		ArrayDeque<Integer> pathToNextNode = new ArrayDeque<Integer>();
		boolean[][] visited = new boolean[30][30];
		visited[state.agent_x_position][state.agent_y_position] = true;
		
		nodes.add(new Node(state.agent_x_position, state.agent_y_position, state.agent_direction, new ArrayDeque<Integer>()));
		while(!nodes.isEmpty()) {
			Node currentNode = nodes.removeFirst();
			if (state.world[currentNode.getX()][currentNode.getY()] == state.UNKNOWN) {
				instructions = currentNode.getList();
				System.out.println("GOAL NODE");
				System.out.println("GOAL NODE");
				System.out.println("Agent X: " + state.agent_x_position);
				System.out.println("Agent Y: " + state.agent_y_position);
				System.out.println("Agent direction: " + state.agent_direction);
				System.out.println("GOAL NODE");
				System.out.println("GOAL NODE");
				System.out.println("Xgoal: " + currentNode.getX());
				System.out.println("Ygoal: " + currentNode.getY());
			...
			case MyAgentState.WEST:
				...
				if(!visited[currentNode.getX()-1][currentNode.getY()] && state.world[currentNode.getX()-1][currentNode.getY()] != state.WALL) {
					pathToNextNode = currentNode.getList();
					pathToNextNode.offer(state.ACTION_MOVE_FORWARD);
					Node addNodetoQueue = new Node(currentNode.getX()-1,currentNode.getY(), currentNode.getDirection(), pathToNextNode);
					nodes.offer(addNodetoQueue);
				...
				if(!visited[currentNode.getX()][currentNode.getY()-1] && state.world[currentNode.getX()][currentNode.getY()-1] != state.WALL) {
					pathToNextNode = currentNode.getList();
					pathToNextNode.offer(state.ACTION_TURN_RIGHT);
					pathToNextNode.offer(state.ACTION_MOVE_FORWARD);
				...
				if(!visited[currentNode.getX()][currentNode.getY()+1] && state.world[currentNode.getX()][currentNode.getY()-1] != state.WALL) {
					pathToNextNode = currentNode.getList();
					pathToNextNode.offer(state.ACTION_TURN_LEFT);
					pathToNextNode.offer(state.ACTION_MOVE_FORWARD);
